...
async function fetchRepoAndAnalysisFromServer(owner: string, repoName: string, forceRefresh = false): Promise<ApiResponse> {
...
export default function RepoDetailsClient({ owner, repoName }: { owner: string; repoName: string }) {
  ...
  if (data?.error === 'ANALYSIS_IN_PROGRESS' || (error?.message.includes('Failed to analyze repository: Internal Server Error'))) {
    return (
      <AnalysisInProgress 
        owner={owner} 
        repoName={repoName} 
